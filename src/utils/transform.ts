...
> {
  ...
  ts.forEachChild(ast, (node) =>
    walkIdentifiers(ts, node, ast, varCb, localVars)
  );
  ...
  if (ctxVars.length) {
      ...
      if (vueOptions.experimentalUseElementAccessInTemplate) {
        ...
        if (ctxVars[i + 1].isShorthand) {
          yield [
            code.substring(
              varEnd,
              ctxVars[i + 1].offset + ctxVars[i + 1].text.length
            ),
            varEnd,
          ];
          yield [": ", undefined];
        ...
        if (ctxVars[i + 1].isShorthand) {
          ...
          yield [
            code.substring(ctxVars[i].offset, ctxVars[i + 1].offset),
            ctxVars[i].offset,
          ];
        }
      }
    ...
    if (vueOptions.experimentalUseElementAccessInTemplate) {
      ...
      const varEnd =
        ctxVars[ctxVars.length - 1].offset +
        ctxVars[ctxVars.length - 1].text.length;
      yield ["__VLS_ctx[", undefined];
      yield ["", varStart, true];
      yield ["'", undefined];
      yield [code.substring(varStart, varEnd), varStart];
      ...
      yield ["__VLS_ctx.", undefined];
      yield [
        code.substring(ctxVars[ctxVars.length - 1].offset),
        ctxVars[ctxVars.length - 1].offset,
      ];
    }
  } else {
...
function walkIdentifiers(
  ...
  if (ts.isIdentifier(node)) {
    ...
    ts.forEachChild(node, (node) =>
      walkIdentifiersInTypeReference(ts, node, cb)
    );
  } else {
    ...
    ts.forEachChild(node, (node) =>
      walkIdentifiers(ts, node, ast, cb, localVars, blockVars, false)
    );
    if (ts.isBlock(node)) {
      for (const varName of blockVars) {
        localVars.set(varName, localVars.get(varName)! - 1);
