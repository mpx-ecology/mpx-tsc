...
export function* generate(
  ...
  const bindingNames = new Set([
    ...(scriptRanges?.bindings.map((range) =>
      script!.content.substring(range.start, range.end)
    ) ?? []),
    ...(scriptSetupRanges?.bindings.map((range) =>
      scriptSetup!.content.substring(range.start, range.end)
    ) ?? []),
  ]);
  const bypassDefineComponent = lang === "js";
      ...
      get code(): string {
        return `type __VLS_WithTemplateSlots<T, S> = T & {
              ...
              vueCompilerOptions.jsxSlots
                ? `$props: ${helperTypes.PropsChildren.name}<S>;`
                : ""
            }
					}
				};`;
  ...
  function* generateGlobalHelperTypes(): Generator<CodeAndStack> {
    const fnPropsType = `(K extends { $props: infer Props } ? Props : any)${
...
function __VLS_directiveFunction<T>(dir: T):
  ...
  }').ObjectDirective<infer E, infer V> | import('${
      vueCompilerOptions.lib
    }').FunctionDirective<infer E, infer V> ? (value: V) => void
	: T;
...
function __VLS_asFunctionalComponent<T, K = T extends new (...args: any) => any ? InstanceType<T> : unknown>(t: T, instance?: K):
	...
	? (props: ${fnPropsType}, ctx?: any) => __VLS_Element & { __ctx?: {
		attrs?: any,
		slots?: K extends { ${getSlotsPropertyName(
      vueCompilerOptions.target
    )}: infer Slots } ? Slots : any,
	...
	: (_: {}${
    vueCompilerOptions.strictTemplates ? "" : " & Record<string, unknown>"
  }, ctx?: any) => { __ctx?: { attrs?: any, expose?: any, slots?: any, emit?: any, props?: {}${
      vueCompilerOptions.strictTemplates ? "" : " & Record<string, unknown>"
    } } };
function __VLS_elementAsFunctionalComponent<T>(t: T): (_: T${
      vueCompilerOptions.strictTemplates ? "" : " & Record<string, unknown>"
    }, ctx?: any) => { __ctx?: { attrs?: any, expose?: any, slots?: any, emit?: any, props?: T${
      vueCompilerOptions.strictTemplates ? "" : " & Record<string, unknown>"
    } } };
...
`);
  ...
  function* generateScriptContentBeforeExportDefault(): Generator<CodeAndStack> {
    ...
    if (scriptRanges?.exportDefault)
      isExportRawObject = script.content
        .substring(
          scriptRanges.exportDefault.expression.start,
    ...
    yield _(
      generateSourceCode(script, 0, scriptRanges.exportDefault.expression.start)
    );
  ...
  function* generateModelEmits(): Generator<CodeAndStack> {
    ...
    if (scriptSetupRanges.defineProp.length) {
      yield _(
        ` & ReturnType<typeof import('${vueCompilerOptions.lib}').defineEmits<{\n`
      );
      for (const defineProp of scriptSetupRanges.defineProp) {
        if (!defineProp.isModel) continue;

        let propName = "modelValue";
        if (defineProp.name) {
          propName = scriptSetup.content.substring(
  ...
  function* generateScriptSetupAndTemplate(): Generator<CodeAndStack> {
    ...
    if (scriptSetup.generic) {
      ...
      if (scriptSetupRanges.props.define?.arg) {
        yield _(`	props: `);
        yield _(
          generateSourceCodeForExtraReference(
            scriptSetup,
      ...
      if (scriptSetupRanges.defineProp.length) {
        ...
        for (const defineProp of scriptSetupRanges.defineProp) {
          if (defineProp.defaultValue) {
            ...
            yield _(
              scriptSetup.content.substring(
                defineProp.defaultValue.start,
                defineProp.defaultValue.end
              )
            );
            yield _(`,\n`);
          }
      ...
      if (scriptSetupRanges.props.define?.typeArg) {
        ...
        yield _(
          generateSourceCode(
            scriptSetup,
            scriptSetupRanges.props.define.typeArg.start,
            scriptSetupRanges.props.define.typeArg.end
          )
        );
      }
      if (scriptSetupRanges.defineProp.length) {
      ...
      yield _(
        `		return {} as {\n` +
          ...
          `			expose(exposed: import('${
            ...
            scriptSetupRanges.expose.define ? "typeof __VLS_exposed" : "{}"
          }>): void,\n` +
          `			attrs: any,\n` +
          `			slots: ReturnType<typeof __VLS_template>,\n` +
          `			emit: typeof ${
            scriptSetupRanges.emits.name ?? "__VLS_emit"
          } & typeof __VLS_modelEmitsType,\n` +
          `		};\n`
      );
      yield _(`	})(),\n`); // __VLS_setup = (async () => {
      yield _(
        `) => ({} as import('${vueCompilerOptions.lib}').VNode & { __ctx?: Awaited<typeof __VLS_setup> }))`
      );
    } else if (!script) {
    ...
    if (scriptSetupGeneratedOffset !== undefined) {
      for (const defineProp of scriptSetupRanges.defineProp) {
        ...
        const propName = scriptSetup.content.substring(
          defineProp.name.start,
          defineProp.name.end
        );
  ...
  function* generateSetupFunction(
    functional: boolean,
    mode: "return" | "export" | "none",
    definePropMirrors: Map<string, number>
  ): Generator<CodeAndStack> {
    if (!scriptSetupRanges || !scriptSetup) return;

    const definePropProposalA =
      scriptSetup.content
        .trimStart()
        .startsWith("// @experimentalDefinePropProposal=kevinEdition") ||
      vueCompilerOptions.experimentalDefinePropProposal === "kevinEdition";
    const definePropProposalB =
      scriptSetup.content
        .trimStart()
        .startsWith("// @experimentalDefinePropProposal=johnsonEdition") ||
    ...
    scriptSetupGeneratedOffset =
      getGeneratedLength() - scriptSetupRanges.importSectionEndOffset;
    ...
    if (scriptSetupRanges.emits.define && !scriptSetupRanges.emits.name) {
      setupCodeModifies.push([
        [`const __VLS_emit = `],
        scriptSetupRanges.emits.define.start,
        scriptSetupRanges.emits.define.start,
      ]);
    }
      ...
      if (scriptSetupRanges.expose.define?.typeArg) {
        setupCodeModifies.push([
          [
            ...
            generateSourceCodeForExtraReference(
              scriptSetup,
              scriptSetupRanges.expose.define.typeArg.start,
              scriptSetupRanges.expose.define.typeArg.end
            ),
            `;\n`,
          ],
        ...
        setupCodeModifies.push([
          [
            ...
            generateSourceCodeForExtraReference(
              scriptSetup,
              scriptSetupRanges.expose.define.arg.start,
              scriptSetupRanges.expose.define.arg.end
            ),
            `;\n`,
          ],
          scriptSetupRanges.expose.define.start,
    ...
    if (
      scriptSetupRanges.props.define?.typeArg &&
      scriptSetupRanges.props.withDefaults?.arg
    ) {
    ...
    if (!functional && scriptSetupRanges.defineProp.length) {
      ...
      for (const defineProp of scriptSetupRanges.defineProp) {
        ...
        if (!defineProp.nameIsString) {
          ...
          type = scriptSetup.content.substring(
            defineProp.type.start,
            defineProp.type.end
          );
        }
        ...
        if (defineProp.required) {
          yield _(
            `{ required: true, type: import('${vueCompilerOptions.lib}').PropType<${type}> },\n`
          );
        } else {
    ...
    if (mode === "return" || mode === "export") {
      if (
        !vueCompilerOptions.skipTemplateCodegen &&
        (templateCodegen?.hasSlot || scriptSetupRanges?.slots.define)
      ) {
        ...
        yield _(
          `{} as ${helperTypes.WithTemplateSlots.name}<typeof __VLS_component, ReturnType<typeof __VLS_template>>;\n`
        );
      } else {
        yield _(mode === "return" ? "return " : "export default ");
        yield* generateComponent(functional);
  ...
  function* generateComponent(functional: boolean): Generator<CodeAndStack> {
    if (!scriptSetupRanges) return;

    if (
      script &&
      scriptRanges?.exportDefault &&
      scriptRanges.exportDefault.expression.start !==
        scriptRanges.exportDefault.args.start
    ) {
  ...
  function* generateComponentOptions(
    ...
    if (script && scriptRanges?.exportDefault?.args) {
      yield _(
        generateSourceCode(
          script,
          scriptRanges.exportDefault.args.start + 1,
          scriptRanges.exportDefault.args.end - 1
        )
      );
    }
  }
  function* generateSetupReturns(): Generator<CodeAndStack> {
    if (scriptSetupRanges && bypassDefineComponent) {
      ...
      if (scriptSetupRanges.props.define) {
        ...
        yield _(
          `$props: __VLS_makeOptional(${
            scriptSetupRanges.props.name ?? `__VLS_props`
          }),\n`
        );
  ...
  function* generateComponentForTemplateUsage(
    functional: boolean,
    cssIds: Set<string>
  ): Generator<CodeAndStack> {
    if (scriptSetup && scriptSetupRanges) {
      yield _(
        `const __VLS_internalComponent = (await import('${vueCompilerOptions.lib}')).defineComponent({\n`
      );
      yield _(`setup() {\n`);
      yield _(`return {\n`);
      yield* generateSetupReturns();
      ...
      yield _(
        `let __VLS_internalComponent!: typeof import('./${path.basename(
          fileName
        )}')['default'];\n`
      );
    } else {
  ...
  function* generateConstNameOption(): Generator<CodeAndStack> {
    ...
    if (script && scriptRanges?.exportDefault?.nameOption) {
      ...
      yield _(
        `${script.content.substring(nameOption.start, nameOption.end)} as const`
      );
      yield _(`;\n`);
    } else if (scriptSetup) {
      yield _(
        `let __VLS_name!: '${path.basename(
  ...
  function* generateTemplateContext(
    cssIds = new Set<string>()
  ): Generator<CodeAndStack> {
    const useGlobalThisTypeInCtx = fileName.endsWith(".html");

    yield _(
      `let __VLS_ctx!: ${useGlobalThisTypeInCtx ? "typeof globalThis &" : ""}`
    );
    yield _(
      `InstanceType<__VLS_PickNotAny<typeof __VLS_internalComponent, new () => {}>> & {\n`
    );
    ...
    for (let i = 0; i < styles.length; i++) {
      ...
      if (style.module) {
        yield _(
          `${style.module}: Record<string, string> & ${helperTypes.Prettify.name}<{}`
        );
    ...
    yield _(
      `let __VLS_own!: __VLS_SelfComponent<typeof __VLS_name, typeof __VLS_internalComponent & (new () => { ${getSlotsPropertyName(
        vueCompilerOptions.target
      )}: typeof ${scriptSetupRanges?.slots?.name ?? "__VLS_slots"} })>;\n`
    );
    yield _(
    ...
    if (templateCodegen) {
      for (let i = 0; i < templateCodegen.tsCodes.length; i++) {
        yield [templateCodegen.tsCodes[i], templateCodegen.tsCodegenStacks[i]];
      }
    } else {
      yield _(`// no template\n`);
  ...
  function getTemplateUsageVars() {
    ...
    if (templateCodegen) {
      ...
      for (const varName of bindingNames) {
        if (
          templateCodegen.tagNames.has(varName) ||
          templateCodegen.tagNames.has(hyphenateTag(varName))
        ) {
  ...
  function generateSourceCodeForExtraReference(
    block: SfcBlock,
    start: number,
    end: number
  ): Code {
