...
const presetInfos = {
  ...
  event: disableAllFeatures({
    semantic: { shouldHighlight: () => false },
    verification: true,
  }),
  tagReference: disableAllFeatures({
    navigation: { shouldRename: () => false },
  }),
  attr: disableAllFeatures({
    semantic: { shouldHighlight: () => false },
    verification: true,
    navigation: true,
  }),
  ...
  slotName: disableAllFeatures({
    semantic: { shouldHighlight: () => false },
    verification: true,
    navigation: true,
    completion: true,
  }),
  slotNameExport: disableAllFeatures({
    semantic: { shouldHighlight: () => false },
    verification: true,
    navigation: true /* __navigationCodeLens: true */,
  }),
...
type _CodeAndStack = [
  codeType: "ts" | "tsFormat" | "inlineCss",
...
export function* generate(
  ...
  const _ts = codegenStack
    ? (code: Code): _CodeAndStack => [
        "ts",
        processDirectiveComment(code),
        getStack(),
      ]
          ...
          if (
            ...
            offsets.push(
              prop.value.loc.start.offset +
                prop.value.loc.source.lastIndexOf(tag)
            );
            break;
          }
        }
  ...
  function* generateSlotsType(): Generator<_CodeAndStack> {
    for (const [exp, slot] of slotExps) {
      hasSlot = true;
      yield _ts(
        `Partial<Record<NonNullable<typeof ${exp}>, (_: typeof ${slot.varName}) => any>> &\n`
      );
    }
    yield _ts(`{\n`);
      ...
      if (slot.name && slot.loc !== undefined) {
        yield* generateObjectProperty(
          ...
          mergeFeatureSettings(
            ...
            disableAllFeatures({ __referencesCodeLens: true })
          ),
          slot.nodeLoc
        );
      } else {
  ...
  function* generatePreResolveComponents(): Generator<_CodeAndStack> {
    ...
    for (const [tagName] of tagOffsetsMap) {
      if (nativeTags.has(tagName)) continue;

      const isNamespacedTag = tagName.indexOf(".") >= 0;
      if (isNamespacedTag) continue;

      yield _ts(
        `& __VLS_WithComponent<'${getCanonicalComponentName(
          tagName
    ...
    for (const [tagName, tagOffsets] of tagOffsetsMap) {
        ...
        if (nativeTags.has(tagName)) {
          ...
          yield* generatePropertyAccess(
            ...
            mergeFeatureSettings(
              ...
              ...[presetInfos.tagHover, presetInfos.diagnosticOnly]
            )
          );
          yield _ts(";");
        } else if (validTsVarReg.test(camelize(tagName))) {
          for (const shouldCapitalize of tagName[0] === tagName.toUpperCase()
            ? [false]
            : [true, false]) {
            const expectName = shouldCapitalize
            ...
            yield* generateCamelized(
              ...
              tagOffset,
              mergeFeatureSettings(presetInfos.tagReference, {
                navigation: {
                  resolveRenameNewName:
                    tagName !== expectName ? camelizeComponentName : undefined,
      ...
      if (!nativeTags.has(tagName) && validTsVarReg.test(camelize(tagName))) {
        yield _ts("// @ts-ignore\n"); // #2304
        yield _ts("[");
        for (const tagOffset of tagOffsets) {
          yield* generateCamelized(
  ...
  function* generateAstNode(
    ...
    if (node.type === CompilerDOM.NodeTypes.ROOT) {
      ...
      while (
        (leftCharacter = template.content.substring(
          start - 1,
          start
        )).trim() === "" &&
      ...
      while (
        (rightCharacter = template.content.substring(
  ...
  function* generateVIf(
    ...
    for (let i = 0; i < node.branches.length; i++) {
      ...
      if (i === 0) yield _ts("if");
      else if (branch.condition) yield _ts("else if");
      else yield _ts("else");
      ...
      if (addedBlockCondition) {
        blockConditions[blockConditions.length - 1] = `!(${
          blockConditions[blockConditions.length - 1]
        })`;
      }
    }
  ...
  function* generateElement(
    ...
    const startTagOffset =
      ...
      template.content.substring(node.loc.start.offset).indexOf(node.tag);
    let endTagOffset =
      !node.isSelfClosing && template.lang === "html"
        ? node.loc.start.offset + node.loc.source.lastIndexOf(node.tag)
        : undefined;
    ...
    if (isIntrinsicElement) {
      ...
      yield _ts(`const ${var_originalComponent} = `);
      yield* generateInterpolation(
        dynamicTagExp.loc.source,
        dynamicTagExp.loc,
        dynamicTagExp.loc.start.offset,
        presetInfos.all,
        "(",
        ")"
      );
    ...
    if (isIntrinsicElement) {
      yield _ts(
        `const ${var_functionalComponent} = __VLS_elementAsFunctionalComponent(${var_originalComponent});\n`
      );
    } else {
      yield _ts(
    ...
    for (const offset of tagOffsets) {
      ...
      yield* generateCanonicalComponentName(
        tag,
        offset,
        mergeFeatureSettings(presetInfos.tagHover, presetInfos.diagnosticOnly)
      );
      yield _ts(";\n");
    }
    ...
    if (vueCompilerOptions.strictTemplates) {
      ...
      yield _ts(
        tagOffsets.length
          ? ["", "template", tagOffsets[0], presetInfos.diagnosticOnly]
          : dynamicTagExp
    ...
    if (slotDir && componentCtxVar) {
      ...
      if (
        ...
        yield _ts([
          ...
          { ...presetInfos.slotName, completion: false },
        ] satisfies Code);
        yield _ts("default");
        yield _ts([
          "",
    ...
    if (
      ...
      usedComponentCtxVars.has(defineComponentCtxVar)
    ) {
      yield _ts(
        `const ${componentCtxVar} = __VLS_pickFunctionalComponentCtx(${var_originalComponent}, ${var_componentInstance})!;\n`
      );
    }
    if (usedComponentEventsVar) {
      yield _ts(
        `let ${componentEventsVar}!: __VLS_NormalizeEmits<typeof ${componentCtxVar}.emit>;\n`
      );
    }
    ...
    for (const prop of node.props) {
      if (
        ...
        const startCode: Code = [
          ...
          mergeFeatureSettings(presetInfos.attrReference, {
            navigation: {
  ...
  function* generateProps(
    ...
    if (mode === "extraReferences") {
      ...
      caps_diagnosticOnly = disableAllFeatures({
        navigation: caps_diagnosticOnly.navigation,
      });
    ...
    for (const prop of props) {
      if (
        ...
        if (
          propName === undefined ||
          vueCompilerOptions.dataAttributes.some((pattern) =>
        ...
        const shouldCamelize =
          ...
          (!prop.arg ||
            (prop.arg.type === CompilerDOM.NodeTypes.SIMPLE_EXPRESSION &&
              prop.arg.isStatic)) && // isStatic
          hyphenateAttr(propName) === propName &&
          !vueCompilerOptions.htmlAttributes.some((pattern) =>
            minimatch(propName!, pattern)
          );
        ...
        yield* generateObjectProperty(
          propName,
          prop.arg ? prop.arg.loc.start.offset : prop.loc.start.offset,
          prop.arg
            ? mergeFeatureSettings(caps_attr, {
                navigation: caps_attr.navigation
                  ? {
                      resolveRenameNewName: camelize,
                      resolveRenameEditText: shouldCamelize
                        ? hyphenateAttr
                        : undefined,
                    }
                  : undefined,
      ...
      if (
        ...
        if (
          ...
          yield* generateInterpolation(
            ...
            prop.arg.loc,
            prop.arg.loc.start.offset +
              prop.arg.loc.source.indexOf(prop.arg.content),
            presetInfos.all,
            "(",
            ")"
          );
        ...
        yield* generateCamelized(
          "v-" + prop.name,
          prop.loc.start.offset,
          mergeFeatureSettings(presetInfos.noDiagnostics, {
            completion: {
  ...
  function* generateSlot(
    ...
    if (hasScriptSetupSlots) {
      ...
      yield _ts(
        slotNameExpNode?.content ??
          `('${getSlotName()?.[0] ?? "default"}' as const)`
      );
      yield _ts([
        "",
        "template",
    ...
    for (const prop of node.props) {
      if (
        ...
        yield* generateObjectProperty(
          prop.arg.content,
          prop.arg.loc.start.offset,
          mergeFeatureSettings(presetInfos.slotProp, {
            navigation: {
              resolveRenameNewName: camelize,
        ...
        yield* generateObjectProperty(
          prop.name,
          prop.loc.start.offset,
          mergeFeatureSettings(presetInfos.attr, {
            navigation: {
              resolveRenameNewName: camelize,
              resolveRenameEditText: getPropRenameApply(prop.name),
            },
        ...
        yield _ts(
          prop.value !== undefined
            ? `"${
                needToUnicode(prop.value.content)
                  ? toUnicode(prop.value.content)
                  : prop.value.content
              }"`
            : "true"
        );
    ...
    function getSlotName() {
      for (const prop2 of node.props) {
        if (
          prop2.name === "name" &&
          prop2.type === CompilerDOM.NodeTypes.ATTRIBUTE &&
          prop2.value
        ) {
          if (prop2.value.content) {
            return [
              prop2.value.content,
              prop2.loc.start.offset +
                prop2.loc.source.indexOf(
                  prop2.value.content,
                  prop2.name.length
                ),
            ] as const;
    ...
    function getSlotNameExpNode() {
      for (const prop2 of node.props) {
        if (
          prop2.type === CompilerDOM.NodeTypes.DIRECTIVE &&
          prop2.name === "bind" &&
          prop2.arg?.type === CompilerDOM.NodeTypes.SIMPLE_EXPRESSION &&
          prop2.arg.content === "name"
        ) {
  ...
  function* generateCamelized(
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (part !== "") {
        yield _ts([
          i === 0 ? part : capitalize(part),
          "template",
          offset,
          i === 0 ? info : disableAllFeatures({ __combineLastMapping: true }),
        ]);
      }
  ...
  function* generateTsFormat(
    code: string,
    offset: number,
    formatWrapper: [string, string]
  ): Generator<_CodeAndStack> {
    ...
    yield _tsFormat([
      ...
      "template",
      offset,
      mergeFeatureSettings(presetInfos.disabledAll, {
        format: true,
  ...
  function* generatePropertyAccess(
    ...
    if (
      ...
      validTsVarReg.test(code)
    ) {
      yield _ts(".");
      yield _ts(
        offset !== undefined && info ? [code, "template", offset, info] : code
      );
  ...
  function* generateStringLiteralKey(
    code: string,
    offset?: number,
    info?: VueCodeInformation
  ): Generator<_CodeAndStack> {
...
export function* eachElementNode(
...
function getModelValuePropName(
  node: CompilerDOM.ElementNode,
  vueVersion: number,
  vueCompilerOptions: VueCompilerOptions
) {
