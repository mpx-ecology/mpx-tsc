      ...
      if (embeddedFile.id.startsWith("script_")) {
        ...
        if (tsx) {
          const [content, contentStacks] = ctx.codegenStack
            ? track([...tsx.codes], [...tsx.codeStacks])
            : [[...tsx.codes], [...tsx.codeStacks]];
        ...
        if (template) {
          const [content, contentStacks] = ctx.codegenStack
            ? track(
                [...template.formatCodes],
                template.formatCodeStacks.map((stack) => ({ stack, length: 1 }))
              )
            : [
                [...template.formatCodes],
                template.formatCodeStacks.map((stack) => ({
                  stack,
                  length: 1,
                })),
              ];
        ...
        if (template) {
          const [content, contentStacks] = ctx.codegenStack
            ? track(
                [...template.cssCodes],
                template.cssCodeStacks.map((stack) => ({ stack, length: 1 }))
              )
            : [
                [...template.cssCodes],
                template.cssCodeStacks.map((stack) => ({ stack, length: 1 })),
...
function createTsx(
  ...
  const lang = computed(() => {
    return !_sfc.script && !_sfc.scriptSetup
      ? "ts"
      : _sfc.scriptSetup && _sfc.scriptSetup.lang !== "js"
      ? _sfc.scriptSetup.lang
      : _sfc.script && _sfc.script.lang !== "js"
      ? _sfc.script.lang
      : "js";
  });
  ...
  const hasScriptSetupSlots = computed(
    () => !!scriptSetupRanges()?.slots.define
  );
